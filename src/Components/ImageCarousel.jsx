...
const VideoCarousel = () => {
  ...
  useEffect(() => {
    if (videoRefs.current[currentIndex]) {
      // Only try to play if it's a video element
  ...
  // Helper function to check if media exists (with fallback)
  const checkMediaExists = async (mediaUrl) => {
    try {
  ...
  return (
    <div className="relative w-full mx-auto">
      {/* Carousel */}
      <div className="relative w-full h-screen overflow-hidden shadow-xl bg-black">
        <div
          ...
          {media.map((item, index) => {
            ...
            return (
              <div
                  ...
                  <>
                    ...
                    {isMediaVideo && (
                      <video
                        ...
                        onError={(e) => {
                          console.error(
                            `Video load error for item ${index}:`,
                            mediaSrc,
                            e
                          );
                ...
                {/* Text Overlay - Uncomment if needed */}
                {/* {(item.title || item.description) && (
                  <div className="absolute inset-0 flex items-center justify-center px-4 sm:px-6 md:px-10 text-center">
                    <div className="bg-black/50 backdrop-blur-sm rounded-lg px-6 py-4 shadow-sm max-w-2xl">
                      {item.title && (
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-5xl max-h-[90vh] overflow-y-auto">
            <div className="relative shadow-2xl">
              {/* Display all media (videos and images) in modal */}
                    ...
                    <div key={idx} className="mb-4">
                      {isModalImage && (
                        <img
                          src={modalMediaSrc}
                          alt={
                            media[modalIndex].title || `Modal image ${idx + 1}`
                          }
                          className="w-full h-auto max-h-[80vh] object-contain bg-black"
                          onError={(e) => {
                            console.error(
                              `Modal image load error ${idx}:`,
                              modalMediaSrc,
                              e
                            );
                          }}
                      ...
                      {isModalVideo && (
                        <video
                          ...
                          onLoadedData={() => {
                            console.log(
                              `Modal video ${idx} loaded successfully`
                            );
                          }}
                        />
                      )}

                      {!isModalImage && !isModalVideo && (
              ...
              {(media[modalIndex].title || media[modalIndex].description) && (
                <div className="mt-4">
